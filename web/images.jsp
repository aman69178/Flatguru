...
Connection con=DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","Aman Singh","king8115747103");		
	String scity=request.getParameter("city").toLowerCase();
...
ResultSet rs=st.executeQuery("SELECT  FLATADD.CITY,FLATADD.STATE,FLATADD.STREET,FLAT.ACCOMADATION,FLAT.RENT,FLAT.ID FROM FLATAdd INNER JOIN FLAT ON FLATADD.ID = FLAT.ID WHERE FLATADD.CITY='" +scity+ "'");
...
//ResultSet rs=st.executeQuery("SELECT  FLATADDRESS.CITY,FLATADDRESS.STATE,FLATADDRESS.STREET,FLAT.ACCOMADATION,FLAT.RENT FROM FLATAddress INNER JOIN FLAT ON FLATADDRESS.ID = FLAT.ID WHERE FLATADDRESS.CITY=scity");
    ...
    <table style="max-height: 230px;min-height: 230px; box-shadow: 2px 2px #888888; 
           display:inline-block;background-color:white;margin-top:23px;margin-left:40px;min-width: 340px;" >
        
        <tr>
     <td> <img style="max-height:228px;min-height:228px; width: 275px;margin-right:5px;" src="view.jsp?id=<%=h%>"></td>
            <td style="max-width: 190px;">
                <table >
   <tr> <font class="pg" style="color: black;font-size: 20px;position:relative;left:11px;top:-34px;"> <%=rs.getString(3)%></font></tr>
